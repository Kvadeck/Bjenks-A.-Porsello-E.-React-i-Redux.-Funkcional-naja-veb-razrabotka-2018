...
// Отобразить элемент React, включая его дочерние элементы, в DOM можно с помощью метода ReactDOM.render. Элемент, который нужно отобразить, передается как
// первый аргумент, а в качестве второго используется целевой узел. В нем должен отобразиться элемент:
...
// В React каждая из этих частей описывается как компонент. Компоненты позволяют многократно применять одну и ту же структуру DOM для различных рецептов
// или наборов данных.

// Компонент React можно создать с помощью функции React.createClass, которая возвращает один элемент неупорядоченного списка, содержащий
// дочернюю запись списка для каждого ингредиента в массиве.
// Список ингредиентов в качестве компонента React
...
// Данные могут передаваться компонентам React в виде свойств. Многократно используемый список ингредиентов можно создать, передавая эти данные списку
// в виде массива:
const IngredientsList = React.createClass({
    displayName: "IngredientsList",
    render() {
...
// Одним из основных свойств, включенных в спецификацию ES6, является React.Component, абстрактный класс, который подходит для создания новых
// компонентов React. Специализированные компоненты могут создаваться путем наследования за счет расширения этого класса с помощью синтаксиса ES6.
